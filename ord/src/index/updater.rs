use {
  self::{
    inscription_updater::InscriptionUpdater, inscription_updater::TX_LIMITS,
  ...
  fn index_block(
    ...
    match writeln!(
      ...
      Err(e) => {
        ...
        return Err(anyhow!("Error writing to log file: {e}"));
      }
    }
    match (log_file.as_ref().unwrap()).flush() {
      Ok(_) => (),
      Err(e) => {
        println!("Error flushing log file: {e}");
        return Err(anyhow!("Error flushing log file: {e}"));
      }
    }

    match Reorg::detect_reorg(&block, self.height, self.index) {
      Ok(_) => (),
      Err(e) => {
        println!("Error detecting reorg: {e}");
        return Err(anyhow!("Error detecting reorg: {e}"));
      }
    }
